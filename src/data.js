const Data = {
  "gettingStarted": {
    "overview": {
      ...
      "compulsary": ["Jio Money Payment Gateway JioMoney will help you in setting up the payment page. The customer is redirected from your website to JioMoney payment page where he/she can complete the transaction by choosing provided payment options such as include debit/credit cards, net banking, other wallets and UPI", "Jio Money Payment Gateway for PCI-DSS compliant Merchants Merchants need to collect the customer card details on their own website and post them to JioMoney. JioMoney processes the transaction at the backend. The customer would not be stopped at JioMoney payment page. Only PCI-DSS compliant merchants are applicable to avail this product.", "Jio Money Wallet In this mode, the customer first selects ‘Pay using JioMoney’ option on the merchant’s website. On selecting, the customer is redirected from merchant’s website to JioMoney’s website where he/she can use JioMoney wallet balance to complete the payment.  In case of insufficient balance, the customer can load the balance from the saved debit/credit cards or net banking to complete the transaction in the same payment flow."]
    },
    "onBoardingProcess": [
    ...
    "howDoesThePaymentProcessWork": {
      "desc": "On the payment checkout page of the\nmerchant’s website, the merchant collects information about the purchase of the\ncustomer. The merchant then redirects the customer to the Jio Money wallet page\nby making an HTTPS POST request to our server. \n\n\n\n \n\n\n\n \n\n\n\nIn the merchant initiated POST REQUEST, one of the\nmandatory parameters is named as checksum. The details of this hash\nparameter have been covered in the later section. But it is absolutely\ncritical for the merchant to calculate the checksum correctly and post to\nus in the request. \n\n\n\n \n\n\n\nWhen the transaction POST REQUEST hits the Jio\nMoney server, a new transaction entry is created in the Jio Money Database. Jio\nMoney creates a unique transaction id to identify the transaction at our end.\nThis identifier is known as the JioMoney Transaction ID. \n\n\n\n \n\n\n\nWith the POST REQUEST, customer would be re-directed to Jio\nMoney’s payment page. Customer pays the amount using his/her wallet balance. If\nthere is an insufficient balance,  he/she\ncan chose other options such as  Credit\nCard, Debit Card, Net Banking, UPI etc to complete the transaction in the same\npayment flow. The customer goes through the necessary\nauthorization/authentication process at bank’s login page, and the bank gives\nthe success/failure response back to Jio Money. \n\n\n\n \n\n\n\nJioMoney marks the transaction status on\nthe basis of response received from Bank. JioMoney provides the final\ntransaction response string to the merchant through a POST RESPONSE. The\nparameters in this response are covered in the subsequent sections. \n\n\n\nIn the POST RESPONSE sent by JioMoney, you\nwould receive the final status of the transaction. You will receive the checksum\nparameter here also. Similar to step 3, it is absolutely crucial to\nverify this checksum value at your end and then only accept/reject the order.\nThis is done to strictly avoid any tampering attempt by the user."
    }
  },
  "webIntegration": {
    "checkoutButton": {
          ...
          "description": [
            {
              ...
              "text": "Checksum should be passed along with request. 458712578754 ",
              "datatype": ["alphanumeric"],
              "lengthd": 0,
              "minLength": 0,
              "maxLength": 0
